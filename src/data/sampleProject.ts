...
export const createProject = (brief: Brief = defaultBrief(), rawScript = defaultScript): Project => {
  ...
  const voiceRoles = VoiceCastingAgent.defaultRoles();
  const assignedScript = {
    ...script,
    lines: script.lines.map((line) => ({
      ...line,
      assignedVoiceRoleId: line.type === "legal" ? "voice-legal" : ["music", "sound-effect", "pause", "note"].includes(line.type) ? undefined : "voice-announcer",
    })),
  };
  const base: Project = {
    id: createId("project"),
...
export const updateScriptFromText = (project: Project, rawScript: string) => {
  ...
  return recomputeProject({ ...project, script, soundCues: [], musicCues: [] }, "Script parsed");
};
