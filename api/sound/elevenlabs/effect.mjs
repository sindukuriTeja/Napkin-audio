...
export default async function handler(req, res) {
  ...
  if (!body.text || !body.text.trim()) {
    res.writeHead(400, { "Content-Type": "application/json", ...corsHeaders(origin) });
    return res.end(JSON.stringify({ error: "Missing required sound effect prompt text." }));
  }

  try {
    const providerResponse = await fetch(buildElevenLabsUrl("/sound-generation", body.outputFormat || "mp3_44100_128"), {
      method: "POST",
    ...
    if (!providerResponse.ok) {
      ...
      return res.end(JSON.stringify({ error: "ElevenLabs sound effect request failed.", detail: detail.slice(0, 500) }));
    }
